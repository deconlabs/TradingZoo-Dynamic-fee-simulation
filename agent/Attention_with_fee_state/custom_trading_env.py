...
class TradingEnv:
    ...
    def _long_cover(self, current_price_mean, current_mkt_position, action):  # Used once in `step()`
        ...
        self.chg_reward[:] = ((self.chg_price * (1 - self.fee_rate) - self.chg_price_mean)
                              * n_stock) * self.chg_makereal / self.initial_budget
        self.chg_posi_var[:1] = -n_stock
        self.chg_posi_entry_cover[:1] = -1
    ...
    def step(self, action):
        ...
        if self.step_st + self.obs_len + self.step_len >= len(self.price):
            ...
            if current_mkt_position != 0 and self.sell_at_end:
                ...
                self.chg_reward[:] = (self.chg_price * (1 - self.fee_rate) - self.chg_price_mean) * \
                    current_mkt_position * self.chg_makereal / self.initial_budget
        ...
        self.chg_reward_fluctuant[:] = (
            self.chg_price * (1 - self.fee_rate) - self.chg_price_mean) * self.chg_posi / self.initial_budget
